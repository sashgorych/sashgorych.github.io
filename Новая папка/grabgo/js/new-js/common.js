...
(function init() {
    document.querySelectorAll('.ChangeTimeOfDay').forEach(function (el) {
        return el.classList.add("night");
    });
    document.querySelectorAll('.ChangeTimeOfDay').forEach(function (el) {
        return el.classList.remove("night");
    });
    setTimeout(function () {
...
document.querySelector(".changeDay").onclick = function () {
    var blocks = document.querySelectorAll('.ChangeTimeOfDay').forEach(function (el) {
        return el.classList.toggle("night");
    });
};

//change food
var blockFood = document.querySelectorAll(".screen2_c1");
var allIcon = document.querySelectorAll(".foods>ul>li");
var foodText = document.querySelectorAll('.text');
document.querySelector('.foods').onclick = function () {
    var iconData = event.target.dataset.type;
    iconData && showCorrectFood(iconData, event.target);
};

function showCorrectFood(iconData, icon) {
    allIcon.forEach(function (el) {
        return hide(el);
    });
    blockFood.forEach(function (el) {
        hide(el);
...
var ul = document.querySelector(".slideBar");
var ball = document.querySelector(".btn");
var li = document.querySelector(".slideBar li");
...
var currentDotPosition = document.querySelector(".dot1");
function moveDots() {
    var distanceLeft = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : getComputedStyle(ball).left;
...
function moveAllItems(position) {
    ...
    for (var _key in itemsScreen2) {
        moveElement(itemsScreen2[_key]['item'], -position * 0.75, itemsScreen2[_key]['duration'], 100);
    }
...
function moveElement(el, delta) {
    var duration = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1;
    var e = arguments[3];

    var animation = el.animate({ left: delta * duration + e + "vw" }, 0);
}
